...
function evm_snr = runOFDM(snrRange, stoError, K, L, kp, P, R, T, cfoErrorHz, combEnabled)
    ...
    for snrCnt = 1:length(snrRange)
        ...
        % Initialize STO estimation variables
        maxDelay = 10; % Maximum expected delay (in samples)
        stoEstimate = zeros(1, L); % Array to store STO estimates for each symbol
        ...
        for t = 1:T
            ...
            % STO estimation and correction (only if stoError == 0)
            if stoError == 0
                % Perform STO estimation using cross-correlation
                [stoEstimate(symbolCnt), ~] = estimateSTO(symbolTd, maxDelay);

                % Apply STO correction
                symbolTd_corrected = correctSTO(symbolTd, stoEstimate(symbolCnt));
            else
                % When STO is already handled by stoError (non-zero), proceed normally
                symbolTd_corrected = symbolTd;
            end

            % Apply CFO correction using the function
            symbolCorrected = applyCFOCorrection(symbolTd_corrected, cfoErrorHz, r, symbolCnt);

            % FFT and FFT processing (remove CP)
...
function [stoEstimate, maxCorr] = estimateSTO(symbolTd, maxDelay)
    % Initialize delay and correlation arrays
    delays = -maxDelay:maxDelay;
    corrValues = zeros(size(delays));

    % Compute cross-correlation for each delay
    for idx = 1:length(delays)
        delayedSymbol = circshift(symbolTd, delays(idx));
        corrValues(idx) = abs(sum(conj(symbolTd) .* delayedSymbol));
    end

    % Find delay corresponding to maximum correlation
    [~, maxIdx] = max(corrValues);
    stoEstimate = delays(maxIdx);
    maxCorr = corrValues(maxIdx);
end

function symbolCorrected = correctSTO(symbolTd, stoEstimate)
    % Apply STO correction
    if stoEstimate > 0
        symbolCorrected = symbolTd(stoEstimate+1:end);
    elseif stoEstimate < 0
        symbolCorrected = [zeros(1, -stoEstimate), symbolTd];
    else
        symbolCorrected = symbolTd;
    end
end
